...
# 24, 308 line: 코드 추가
# 311, 399 line: 판정선 및 테두리 위치 와 너비 값, 색상 변경 (코드 수정)
# 변경 이유: 테두리 선 색 변경시 노트와 겹치는 현상 발생
# 435 line: 게임 오버시 노래 종료
# 438 line: 고려중, 프로그램 재시행, 혹은 UIUX.py의 main 부분을 함수화 시켜 import 고려
import subprocess
import sys
...
def rating(n):
    global combo, miss_anim, last_combo, combo_effect, combo_effect2, combo_time, rate, current_health, game_over, game_end, music_length
...
# 디자인================================================================================================================================
    ...
    # 색상 가져오기 (design.py에서 저장된 색상)
        outline_color = get_outline_color()

        # 테두리 사각형 그리기 (게임 옵션에서 선택된 색상으로)
...
# 게이지 테두리
        ...
        for tile_data in t1:
            ...
            pygame.draw.rect(screen, (255, 255, 255), (w / 2 - w / 8, tile_data[0] - h / 100, w / 15, h / 50))
    ...
    # 판정선===========================================================================================================================
        pygame.draw.rect(screen, (0, 0, 0), (785 - w / 8, (h / 12) * 9, 431, h / 2))
        pygame.draw.rect(screen, (255, 255, 255), (785 - w / 8, (h / 12) * 9, 431, h /2), int(h /100))
    #==================================================================================================================================
...
#게임오버=====================================================================================================================================
    if game_over:
        ...
        #subprocess.run(["python", "UIUX.py"])
        main = False  # 게임 종료

# 게임 엔드=================================================================================================================================== 
    if game_end:
        screen.fill((0, 0, 0))
        game_end_font = pygame.font.Font(None, 72)
        game_end_text = game_end_font.render("GAME END", True, (255, 255, 255))
        screen.blit(game_end_text, (w / 2 - game_end_text.get_width() / 2, h / 2 - game_end_text.get_height() / 2))
        pygame.display.flip()
